  ...
  <q-page class="q-pa-md page-container">
    <q-card flat bordered class="col b-card">
      <q-card-section class="q-gutter-md text-center title-section">
        <div class="text-h5 text-bold page-title">Encryption Generator</div>
      ...
      <q-card-section class="q-gutter-md form-section">
        ...
        <div class="button-wrapper">
          <q-btn
            ...
            @click="encrypt()"
          >
            <q-icon name="mdi-lock-plus" class="btn-icon" />
          </q-btn>
        </div>
      </q-card-section>
    </q-card>
...
const encrypt = () => {
  if (!plainText.value) {
    result.value = 'Please enter text to encrypt';
    return;
  }
  if (!secretKey.value) {
    result.value = 'Please enter a secret key';
    return;
  }
  ...
  try {
    ...
    switch (algorithm.value) {
      case 'AES':
        encrypted = CryptoJS.AES.encrypt(plainText.value, secretKey.value).toString();
        break;
      case 'DES':
        encrypted = CryptoJS.DES.encrypt(plainText.value, secretKey.value).toString();
        break;
      case 'RC4':
        encrypted = CryptoJS.RC4.encrypt(plainText.value, secretKey.value).toString();
        break;
      case 'Rabbit':
        encrypted = CryptoJS.Rabbit.encrypt(plainText.value, secretKey.value).toString();
        break;
      case 'TripleDES':
        encrypted = CryptoJS.TripleDES.encrypt(plainText.value, secretKey.value).toString();
        break;
    }
    result.value = encrypted;
  } catch (error) {
    result.value = 'Encryption failed: ' + String(error);
  }
};
